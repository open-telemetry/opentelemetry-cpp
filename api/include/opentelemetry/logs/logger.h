...
public:
  ...
  /**
   * The secondary base Log(...) method that all other Log(...) overloaded methods except the one above
   * will eventually call,  in order to create a log record.
   * 
  ...
  {
    return this->Log(
        timestamp, severity, name, body,
        nostd::span<const std::pair<nostd::string_view, common::AttributeValue>>{resource.begin(),
                                                                                 resource.end()},
        nostd::span<const std::pair<nostd::string_view, common::AttributeValue>>{attributes.begin(),
                                                                                 attributes.end()}, trace_id, span_id, trace_flags);
  }
  ...
  {
    this->Log(time, severity, default_name_, message, {}, {}, default_trace_id_, default_span_id_, default_trace_flags_);
  }
  ...
  {
    this->Log(default_severity_, message); 
  }
  ...
  // {
  //   this->Log(default_timestamp_, severity, name, default_body_, default_resource_, attributes, default_trace_id_, default_span_id_, default_trace_flags_);
  // }
