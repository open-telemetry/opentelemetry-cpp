...
namespace SemanticConventions
{
  ...
  *
  * <p>Notes:
    <ul> <li>If setting a {@code db.mssql.instance_name}, {@code net.peer.port} is no longer required (but still recommended if non-standard).</li> </ul>
  */
  ...
  */
  static constexpr const char* DB_CASSANDRA_SPECULATIVE_EXECUTION_COUNT = "db.cassandra.speculative_execution_count";
...
as done in the <a href="#exception-end-example">example above</a>.</li><li>It follows that an exception may still escape the scope of the span
...
{@code faas.trigger} MUST be set.</li><li>Clients invoking FaaS instances usually cannot set {@code faas.trigger},
...
call to invoke the lambda, which is often HTTP).</li> </ul>
  ...
  */
  ...
  /**
  * The <a href="../../resource/semantic_conventions/README.md#service">{@code service.name}</a> of the remote service. SHOULD be equal to the actual {@code service.name} resource attribute of the remote service if any.
  */
  ...
  */
  ...
  /**
  * Full HTTP request URL in the form {@code scheme://host[:port]/path?query[#fragment]}. Usually the fragment is not transmitted over HTTP, but if it is known, it should be included nevertheless.
  *
  * <p>Notes:
    <ul> <li>{@code http.url} MUST NOT contain credentials passed via URL in form of {@code https://username:password@www.example.com/}. In such case the attribute's value should be {@code https://www.example.com/}.</li> </ul>
  */
  ...
  *
  * <p>Notes:
    <ul> <li>If {@code net.transport} is not specified, it can be assumed to be {@code IP.TCP} except if {@code http.flavor} is {@code QUIC}, in which case {@code IP.UDP} is assumed.</li> </ul>
  */
  ...
  */
  static constexpr const char* HTTP_REQUEST_CONTENT_LENGTH_UNCOMPRESSED = "http.request_content_length_uncompressed";
  ...
  */
  static constexpr const char* HTTP_RESPONSE_CONTENT_LENGTH_UNCOMPRESSED = "http.response_content_length_uncompressed";

  /**
  * The primary server name of the matched virtual host. This should be obtained via configuration. If no such configuration can be obtained, this attribute MUST NOT be set ( {@code net.host.name} should be used instead).
  *
  * <p>Notes:
    <ul> <li>{@code http.url} is usually not readily available on the server side but would have to be assembled in a cumbersome and sometimes lossy process from other information (see e.g. open-telemetry/opentelemetry-python/pull/148). It is thus preferred to supply the raw data that is available.</li> </ul>
  */
...
identify the network-level peer, which may be a proxy.</li><li>This attribute should be set when a source of information different
from the one used for {@code net.peer.ip}, is available even if that other
source just confirms the same value as {@code net.peer.ip}.
Rationale: For {@code net.peer.ip}, one typically does not know if it
...
the closest proxy.</li> </ul>
  ...
  */
  static constexpr const char* AWS_DYNAMODB_ITEM_COLLECTION_METRICS = "aws.dynamodb.item_collection_metrics";

  /**
  * The value of the {@code ProvisionedThroughput.ReadCapacityUnits} request parameter.
  */
  static constexpr const char* AWS_DYNAMODB_PROVISIONED_READ_CAPACITY = "aws.dynamodb.provisioned_read_capacity";

  /**
  * The value of the {@code ProvisionedThroughput.WriteCapacityUnits} request parameter.
  */
  static constexpr const char* AWS_DYNAMODB_PROVISIONED_WRITE_CAPACITY = "aws.dynamodb.provisioned_write_capacity";
  ...
  */
  static constexpr const char* AWS_DYNAMODB_GLOBAL_SECONDARY_INDEXES = "aws.dynamodb.global_secondary_indexes";

  /**
  * The JSON-serialized value of each item of the {@code LocalSecondaryIndexes} request field.
  */
  static constexpr const char* AWS_DYNAMODB_LOCAL_SECONDARY_INDEXES = "aws.dynamodb.local_secondary_indexes";

  /**
  * The value of the {@code ExclusiveStartTableName} request parameter.
  */
  static constexpr const char* AWS_DYNAMODB_EXCLUSIVE_START_TABLE = "aws.dynamodb.exclusive_start_table";
  ...
  */
  static constexpr const char* AWS_DYNAMODB_ATTRIBUTE_DEFINITIONS = "aws.dynamodb.attribute_definitions";

  /**
  * The JSON-serialized value of each item in the the {@code GlobalSecondaryIndexUpdates} request field.
  */
  static constexpr const char* AWS_DYNAMODB_GLOBAL_SECONDARY_INDEX_UPDATES = "aws.dynamodb.global_secondary_index_updates";
  ...
  */
  static constexpr const char* MESSAGING_MESSAGE_PAYLOAD_SIZE_BYTES = "messaging.message_payload_size_bytes";

  /**
  * The compressed size of the message payload in bytes.
  */
  static constexpr const char* MESSAGING_MESSAGE_PAYLOAD_COMPRESSED_SIZE_BYTES = "messaging.message_payload_compressed_size_bytes";
  ...
  */
  ...
  /**
  * The identifier for the consumer receiving a message. For Kafka, set it to {@code {messaging.kafka.consumer_group} - {messaging.kafka.client_id}}, if both are present, or only {@code messaging.kafka.consumer_group}. For brokers, such as RabbitMQ and Artemis, set it to the {@code client_id} of the client consuming the message.
  */
  ...
  */
  ...
  /**
  * Message keys in Kafka are used for grouping alike messages to ensure they're processed on the same partition. They differ from {@code messaging.message_id} in that they're not unique. If the key is {@code null}, the attribute MUST NOT be set.
  *
  * <p>Notes:
  ...
  */
  static constexpr const char* MESSAGING_ROCKETMQ_CONSUMPTION_MODEL = "messaging.rocketmq.consumption_model";
  ...
  */
  ...
  /**
  * Protocol version as in {@code jsonrpc} property of request/response. Since JSON-RPC 1.0 does not specify this, the value can be omitted.
  */
