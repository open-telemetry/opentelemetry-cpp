...
class TraceState
{
  ...
  //  and
  //  // forward slashes /.  For multi-tenant vendor scenarios, an at sign (@) can be used to prefix
  //  the
  //  // vendor name.
...
public:
  ...
  // An empty TraceState.
  TraceState() noexcept {};
  TraceState(TraceState &&trace_state) { tmp_map = trace_state.tmp_map; }
  TraceState(const TraceState &trace_state) { tmp_map = trace_state.tmp_map; }
  ...
  void Set(nostd::string_view key, nostd::string_view value) noexcept { tmp_map[key] = value; }
