...
public:
  ...
  static nostd::shared_ptr<TraceState> FromHeader(nostd::string_view header)
  {
    ...
    // get number of kv entries in header first to allocate required size of kv_properties_
    size_t count = 0;
  ...
  /**
   *  Returns true along with `value` associated with `key` passed as argument
   *  Returns false if key is invalid  or not found
   */
