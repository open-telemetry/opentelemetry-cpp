...
{
public:
  BaggageImpl() : kv_properties_(new opentelemetry::common::KeyValueProperties()) {}
  explicit BaggageImpl(size_t size)
      : kv_properties_(new opentelemetry::common::KeyValueProperties(size)){};

  template <class T>
  BaggageImpl(const T &keys_and_values)
      : kv_properties_(new opentelemetry::common::KeyValueProperties(keys_and_values))
  {}

  static BaggageImpl *GetDefault() { return new BaggageImpl(); }
  ...
  */
  BaggageImpl *Set(const nostd::string_view &key, const nostd::string_view &value)
  {
    auto baggage_impl   = new BaggageImpl(kv_properties_->Size() + 1);
    const bool valid_kv = IsValidKey(key) && IsValidValue(value);

    if (valid_kv)
    {
      baggage_impl->kv_properties_->AddEntry(key, value);
    }

    // add rest of the fields.
    kv_properties_->GetAllEntries(
        [baggage_impl, &key, &valid_kv](nostd::string_view e_key, nostd::string_view e_value) {
          // if key or value was not valid, add all the entries. Add only remaining entries
          // otherwise.
          if (!valid_kv || key != e_key)
          {
            baggage_impl->kv_properties_->AddEntry(e_key, e_value);
          }
  ...
  // first place.
  BaggageImpl *Delete(nostd::string_view key)
  {
    // keeping size of baggage same as key might not be found in it
    auto baggage_impl = new BaggageImpl(kv_properties_->Size());
    kv_properties_->GetAllEntries(
        [baggage_impl, &key](nostd::string_view e_key, nostd::string_view e_value) {
          if (key != e_key)
            baggage_impl->kv_properties_->AddEntry(e_key, e_value);
          return true;
        });
  ...
  static BaggageImpl *FromHeader(nostd::string_view header)
  {
    ...
    auto baggage_impl = new BaggageImpl(cnt);
    bool kv_valid;
    nostd::string_view key, value;

    while (kv_str_tokenizer.next(kv_valid, key, value) &&
           baggage_impl->kv_properties_->Size() < cnt)
    {
      ...
      if (err == false && IsValidKey(key_str) && IsValidValue(value_str))
      {
        ...
        baggage_impl->kv_properties_->AddEntry(key_str, value_str);
      }
    }
