...
public:
  ...
  {
    // Do not call now() when this duration is max value, now() may have a expemsive cost.
    ...
    // mey use steady_clock or system_clock.We need make sure now() + timeout do not overflow.
