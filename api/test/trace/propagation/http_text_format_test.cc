...
TEST(HTTPTextFormatTest, NoSpanTest)
{
    ...
    format.Inject(Setter,carrier,context::Context());
    const context::Context c1 = context::Context();
    std::map<nostd::string_view,nostd::string_view> c2 = format.Extract(Getter,carrier,c1);
