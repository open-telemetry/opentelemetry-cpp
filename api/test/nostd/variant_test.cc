...
{
  EXPECT_TRUE((std::is_same<variant_alternative_t<0, variant<int>>, int>::value));
  EXPECT_TRUE((std::is_same<variant_alternative_t<1, variant<int, double>>, double>::value));
  EXPECT_TRUE(
      (std::is_same<variant_alternative_t<1, const variant<int, double>>, const double>::value));
}
