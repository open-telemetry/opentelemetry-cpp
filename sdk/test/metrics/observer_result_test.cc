...
TEST(ObserverResult, BasicTests)
{
    ...
    observer_result.Observer(30l, opentelemetry::common::KeyValueIterableView<std::map<std::string, int64_t>>(m1));
    EXPECT_EQ(observer_result.data_.size(), 2);

    observer_result.Observer(40l, opentelemetry::common::KeyValueIterableView<std::map<std::string, int64_t>>(m1));
    EXPECT_EQ(observer_result.data_.size(), 2);

    std::map<std::string, int64_t> m2 = {{"k2", 12}, {"k4", 12}};
    observer_result.Observer(40l, opentelemetry::common::KeyValueIterableView<std::map<std::string, int64_t>>(m2));
