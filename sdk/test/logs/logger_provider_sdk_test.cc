...
TEST(LoggerProviderSDK, LoggerEqualityCheck)
{
  ...
  auto logger5 = lp->GetLogger("logger5", "opentelelemtry_library", "1.0.0", schema_url, {{"key", "value"}});
  auto another_logger5 = lp->GetLogger("logger5", "opentelelemtry_library", "1.0.0", schema_url, {{"key", "value"}});
...
TEST(LoggerProviderSDK, GetLoggerInequalityCheck)
{
  ...
  auto logger_attribute_1 = lp->GetLogger("logger1", "library_1", "1.0.0", "url_1", {{"key", "one"}});
  auto logger_attribute_2 = lp->GetLogger("logger1", "library_1", "1.0.0", "url_1", {{"key", "two"}});
