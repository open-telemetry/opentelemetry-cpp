...
TEST(ParentOrElseSampler, ShouldSample)
{
	...
	auto sampling_result2 = sampler_off.ShouldSample(&parent_context_sampled, trace_id, "", span_kind, view);
	ASSERT_EQ(Decision::RECORD_AND_SAMPLE, sampling_result2.decision);

	// Case 3: Parent exists and SampledFlag is false

	auto sampling_result3 = sampler_on.ShouldSample(&parent_context_nonsampled, trace_id, "", span_kind, view);
	ASSERT_EQ(Decision::RECORD_AND_SAMPLE, sampling_result2.decision);
}
