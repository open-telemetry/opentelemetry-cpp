...
public:
  ...
  bool IsExportCompleted() { return *is_export_completed_; }
...
TEST_F(BatchSpanProcessorTestPeer, TestForceFlushTimeout)
{
  ...
  const std::chrono::milliseconds export_delay(600);
  const int num_spans = 3;

  auto batch_processor =
      GetMockProcessor(spans_received, is_shutdown, is_export_completed, export_delay);
