...
public:
  ...
  LastValueAccumulation<T> Merge(LastValueAccumulation<T> &previous,
                                 LastValueAccumulation<T> &current) noexcept override
  {
    auto latest_accumulation = (current.GetLastValueTimeStamp().time_since_epoch() >=
                                previous.GetLastValueTimeStamp().time_since_epoch())
                                   ? current
                                   : previous;

    return new LastValueAccumulation<T>(latest_accumulation.toPointData().value_,
                                        latest_accumulation.GetLastValueTimeStamp());
  }

  /** Returns a new DELTA aggregation by comparing two cumulative measurements.*/
  LastValueAccumulation<T> diff(LastValueAccumulation<T> &prev,
                                LastValueAccumulation<T> &current) noexcept override
  {
