...
{
public:
  Counter(std::unique_ptr<InstrumentDescriptor> && instrument_descriptor, std::unique_ptr<WritableMetricsStorage<T>> && writable_storage):
    instrument_descriptor_{std::move(instrument_descriptor)}, writable_storage_{std::move(writable_storage)}
  {}
