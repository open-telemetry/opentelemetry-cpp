...
std::string RymlDocumentNode::GetRequiredRaw(const std::string &name)
{
  OTEL_INTERNAL_LOG_DEBUG("RymlDocumentNode::GetRequiredRaw(" << name << ")");
  ...
  ryml::ConstNodeRef attr = m_node[name_str];
  if (attr.empty())
  {
    OTEL_INTERNAL_LOG_ERROR("Yaml: empty node: " << name);
    throw InvalidSchemaException(name);
  }

  if (!attr.has_val())
  {
    OTEL_INTERNAL_LOG_ERROR("Yaml: no value node: " << name);
    throw InvalidSchemaException(name);
  }

  ryml::csubstr view = attr.val();
  if (view.len == 0)
  {
    OTEL_INTERNAL_LOG_ERROR("Yaml: 0 length node: " << name);
    throw InvalidSchemaException(name);
  }
...
std::pair<bool, std::string> RymlDocumentNode::GetRaw(const std::string &name)
{
  OTEL_INTERNAL_LOG_DEBUG("RymlDocumentNode::GetRaw(" << name << ")");
