 ...
 */
static size_t utf8_range_ValidateUTF8Naive(const char *data, const char *end, int return_position)
{
  ...
  while (data + codepoint_bytes < end)
  {
    ...
    if (len >= 3)
    {
       ...
       */
      if (!utf8_range_IsTrailByteOk(byte2) || !utf8_range_IsTrailByteOk(byte3))
      {
        return err_pos;
      }
 ...
 */
static inline const char *utf8_range_SkipAscii(const char *data, const char *end)
{
  while (8 <= end - data && (utf8_range_UnalignedLoad64(data) & 0x8080808080808080) == 0)
  {
    data += 8;
  }
...
#  include "src/common/internal/utf8_range/utf8_range_sse.inc"
#elif defined(__ARM_NEON) && defined(__ARM_64BIT_STATE)
#  include "src/common/internal/utf8_range/utf8_range_neon.inc"
#endif

static FORCE_INLINE_ATTR size_t utf8_range_Validate(const char *data,
                                                    size_t len,
                                                    int return_position)
{
  if (len == 0)
    return 1 - return_position;
...
#if defined(__SSE4_1__) || (defined(__ARM_NEON) && defined(__ARM_64BIT_STATE))
  return utf8_range_ValidateUTF8Simd(data_original, data, end, return_position);
#else
  return (return_position ? (data - data_original) : 0) +
         utf8_range_ValidateUTF8Naive(data, end, return_position);
#endif
