    ...
    opentelemetry::sdk::metrics::InstrumentDescriptor &instrument_descriptor) noexcept
{
  auto aggregator =
      (instrument_descriptor.valueType_ == InstrumentValueType.kLong)
          ? std::unique_ptr<Aggregator>(new SumAggregator<long>(instrument_descriptor))
          : std::unique_ptr<Aggregator>(new SumAggregator<double>(instrument_descriptor));
  return std::move(aggregator);
}

static Aggregation &SumAggregation::GetInstance() noexecept
{
