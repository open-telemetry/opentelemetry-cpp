...
void InitMetrics(const std::string &name)
{
  ...
  std::string version;//{"11.2.0"};
  std::string schema;//{"https://opentelemetry.io/schemas/12.2.0"};
  ...
  // histogram view
  {
      ...
      aggr_conf->boundaries_ = std::vector<double>{};
  ...
  std::unique_ptr<opentelemetry::metrics::MeterProvider> api_provider(std::move(provider));
  opentelemetry::metrics::Provider::SetMeterProvider(std::move(api_provider));
}
