...
{
public:
  Result(std::unique_ptr<Response> res, Error err) : res_(std::move(res)), err_(err) {}
...
private:
  ...
  void stop_core();
  std::unique_ptr<Response> send_with_content_provider(const char *method,
                                                       const char *path,
                                                       const Headers &headers,
                                                       const std::string &body,
                                                       size_t content_length,
...
inline bool process_server_socket(socket_t sock,
                                  ...
                                  T callback)
{
  return process_server_socket_core(sock, keep_alive_max_count, keep_alive_timeout_sec,
                                                ...
                                                const std::string &content_type)
{
  ...
  else
  {
    ...
    {
      if (detail::write_content_without_length(strm, res.content_provider_, is_shutting_down) < 0)
      {
        return false;
      }
    }
...
inline bool ClientImpl::write_request(Stream &strm, const Request &req, bool close_connection)
{
  ...
  {
    headers.insert(make_bearer_token_authentication_header(proxy_bearer_token_auth_token_, true));
  }
                                        ...
                                        bool close_connection)
{
  ...
  {
    auto out = req.content_receiver ? static_cast<ContentReceiver>([&](const char *buf, size_t n) {
