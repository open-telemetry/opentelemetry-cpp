...
The OTLP exporter offers some configuration options. To configure the exporter, create an `OtlpExporterOptions` struct (defined in [exporter.h](./include/otlp_exporter.h), set the options inside, and pass the struct to the `OtlpExporter` constructor, like so:
