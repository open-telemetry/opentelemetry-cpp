    ...
    const opentelemetry::nostd::span<std::unique_ptr<sdklogs::Recordable>> &records) noexcept
{
  ...
  {
    OTEL_INTERNAL_LOG_ERROR("[Ostream Log Exporter] Export failed with exception: " << e.what());
    return sdk::common::ExportResult::kFailure;
  }
  catch (...)
  {
    OTEL_INTERNAL_LOG_ERROR("[Ostream Log Exporter] Export failed with unknown exception");
    return sdk::common::ExportResult::kFailure;
  }
#endif
