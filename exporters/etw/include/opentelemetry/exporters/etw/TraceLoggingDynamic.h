...
// TODO: [MG] - verify this header against the latest version in
// //depot/*/publicint/onecoreuapbase/inc/traceloggingdynamic.h
#pragma once
#include <windows.h>
...
registered.
*/
inline HRESULT RegisterProvider(_Out_ REGHANDLE *phProvider,
                                _In_ const GUID *pProviderId,
                                _In_count_x_((UINT16 *)pProviderMetadata)
...
*/
inline HRESULT UnregisterProvider(REGHANDLE hProvider);
...
Example usage:
    ...
    tld::WriteEvent(
        ...
        pDataDescriptors);
*/
inline HRESULT WriteEvent(_In_ REGHANDLE hProvider,
...
*/
inline HRESULT SetInformation(_In_ REGHANDLE hProvider,
...
*/
inline GUID GetGuidForName(_In_z_ char const *szUtf8Name);
...
*/
inline GUID GetGuidForName(_In_z_ wchar_t const *szUtf16Name);
...
*/
template <class ByteBufferTy>
void PushBackAsUtf8(ByteBufferTy &buffer, _In_z_ wchar_t const *szUtf16);
...
#      define TLD_HAVE_EVENT_SET_INFORMATION \
...
#ifndef _TLD_ASSERT
#  if _TLD_DEBUG
#    define _TLD_ASSERT(exp, str)                                                                  \
      ((void)(!(exp)                                                                               \
                  ? (__annotation(L"Debug", L"AssertFail", L"TraceLogging: " L#exp L" : " L##str), \
                     DbgRaiseAssertionFailure(), 0)                                                \
                  : 0))
#  else  // _TLD_DEBUG
...
protected:
  explicit MetadataBuilderBase(ByteBufferTy &buffer) : m_buffer(buffer) { return; }
  ...
  void AddBytes(_In_bytecount_(cb) void const *p, unsigned cb)
  {
  ...
  void AddString(_In_z_ char const *szUtf8)
  {
    for (unsigned i = 0;; i++)
    {
  ...
  void AddString(_In_z_ wchar_t const *szUtf16) { PushBackAsUtf8(m_buffer, szUtf16); }

  void AddU8(UINT8 val)
  {
    m_buffer.push_back(val);
    ;
  }

  void AddU16(UINT16 val)
  {
    m_buffer.push_back(static_cast<UINT8>(val));
    m_buffer.push_back(static_cast<UINT8>(val >> 8));
  }

  UINT32 GetBookmark() { return static_cast<UINT32>(m_buffer.size()); }

  void SetU8(UINT32 bookmark, UINT8 val)
  {
    _TLD_ASSERT(bookmark < m_buffer.size(), "bookmark out of range");
    m_buffer[bookmark] = val;
  }

  void SetU16(UINT32 bookmark, UINT16 val)
  {
  ...
  void IncrementU7(UINT32 bookmark)
  {
    _TLD_ASSERT(bookmark < m_buffer.size(), "bookmark out of range");
    UINT8 result;
...
public:
  ByteBufferTy &GetBuffer() { return m_buffer; }
};
...
public:
  Sha1ForNonSecretPurposes() : m_length(), m_pos()
  {
  ...
  void Append(_In_reads_bytes_(cbInput) void const *pInput, unsigned cbInput)
  {
...
private:
  void Drain()
  {
    ...
    for (unsigned i = 0; i != 20; i++)
    {
      ...
      UINT32 f       = (b & c) | ((~b) & d);
      UINT32 temp    = _rotl(a, 5) + f + e + k + m_w[i];
      e              = d;
      d              = c;
      c              = _rotl(b, 30);
      b              = a;
...
*/
static unsigned CopyUtfCodePoint(_Out_writes_to_(2, return ) wchar_t *pchUtf16Output,
                                 char const *&pszUtf8Input)
{
  ...
  else if (ch <= 0xdf)
  {
    ...
    unsigned ch2 = ((ch & 0x1f) << 6) | (pszUtf8Input[0] & 0x3f);
    if (0x0080 <= ch2)
    {
      ch = ch2;
      pszUtf8Input += 1;
  ...
  else if (ch <= 0xef)
  {
    ...
    unsigned ch2 = ((ch & 0x0f) << 12) | ((pszUtf8Input[0] & 0x3f) << 6) | (pszUtf8Input[1] & 0x3f);
    if (0x0800 <= ch2)
    {
      ch = ch2;
      pszUtf8Input += 2;
  ...
  else if (ch <= 0xf4)
  {
    ...
    unsigned ch2 = ((ch & 0x07) << 18) | ((pszUtf8Input[0] & 0x3f) << 12) |
                   ((pszUtf8Input[1] & 0x3f) << 6) | (pszUtf8Input[2] & 0x3f);
    ch2 -= 0x10000;
    if (ch2 <= 0xfffff)
    {
                                 ...
                                 wchar_t const *&pszUtf16Input)
{
  ...
  pszUtf16Input += 1;
  if (0xd800 <= ch && ch < 0xdc00 && 0xdc00 <= *pszUtf16Input && *pszUtf16Input < 0xe000)
  {
...
__declspec(selectany) extern UINT32 const NamespaceValue[] = {0xB22D2C48, 0xC84790C3, 0x151AF887,
                                                              0xFB30C1Bf};

template <class CharTy>
GUID GetGuidForNameImpl(_In_z_ CharTy const *szUtfName)
{
  /*
  Algorithm:
...
#pragma warning(suppress : 26035)  // string is counted, not nul-terminated.
        LCMapStringEx(LOCALE_NAME_INVARIANT, LCMAP_UPPERCASE | LCMAP_BYTEREV, buffer, iBuffer,
                      buffer, iBuffer, NULL, NULL, 0);
...
enum OutType : UINT8
{
  ...
  OutTypeUtf8       = 0x23,  // Affects narrow string types.
...
*/
struct EventDescriptor : EVENT_DESCRIPTOR
{
  explicit EventDescriptor(UCHAR level       = 5,  // 5 = WINEVENT_LEVEL_VERBOSE
  ...
  void Reset(UCHAR level       = 5,  // 5 = WINEVENT_LEVEL_VERBOSE
             ULONGLONG keyword = 0,  // 0 = no keywords
             UCHAR opcode      = 0,  // 0 = WINEVENT_OPCODE_INFO
             USHORT task       = 0)        // 0 = WINEVENT_TASK_NONE
  {
  ...
  */
  UINT32 Identity() const { return (Version << 16) | Id; }
...
void PushBackAsUtf8(ByteBufferTy &buffer, _In_z_ wchar_t const *szUtf16)
{
  for (unsigned i = 0;; i++)
  {
    ...
    else if (0xd800 <= ch && ch < 0xdc00 && 0xdc00 <= szUtf16[i + 1] && szUtf16[i + 1] < 0xe000)
    {
...
inline ::tld::Type MakeType(::tld::InType inType, ::tld::OutType outType)
{
  return _TLD_MAKE_TYPE(inType & InTypeMask, outType & OutTypeMask);
}

_Use_decl_annotations_ inline GUID GetGuidForName(wchar_t const *szUtf16Name)
{
  return ::tld::detail::GetGuidForNameImpl(szUtf16Name);
}

_Use_decl_annotations_ inline GUID GetGuidForName(char const *szUtf8Name)
{
                                ...
                                _In_opt_ void *pEnableCallbackContext)
{
  ...
  ULONG status = ::EventRegister(pProviderId, pEnableCallback, pEnableCallbackContext, &hProvider);
  if (status == 0)
  {
    auto cbMetadata = *reinterpret_cast<UINT16 const *>(pProviderMetadata);
    status          = tld::SetInformation(hProvider,
                                 static_cast<_EVENT_INFO_CLASS>(2),  // EventProviderSetTraits
                                 const_cast<UINT8 *>(pProviderMetadata), cbMetadata);
...
_Use_decl_annotations_ inline HRESULT SetInformation(REGHANDLE hProvider,
                                                     _EVENT_INFO_CLASS informationClass,
                                                     void *pvInformation,
                                                     ULONG cbInformation)
{
...
#  pragma warning(suppress : 6387)  // It's ok for pvInformation to be null if cbInformation is 0.
  status = TLD_EVENT_SET_INFORMATION(hProvider, informationClass, pvInformation, cbInformation);
      ...
      GetModuleHandleExW(0, L"advapi32.dll", &hEventing))
  {
    typedef ULONG(WINAPI * PFEventSetInformation)(
        _In_ REGHANDLE RegHandle, _In_ enum _EVENT_INFO_CLASS InformationClass,
    ...
    {
      status = pfEventSetInformation(hProvider, informationClass, pvInformation, cbInformation);
    }
...
_Use_decl_annotations_ inline HRESULT WriteEvent(REGHANDLE hProvider,
                                                 ...
                                                 LPCGUID pRelatedActivityId)
{
  ...
  ULONG status = ::EventWriteTransfer(hProvider, &eventDescriptor, pActivityId, pRelatedActivityId,
                                      cDataDescriptors, pDataDescriptors);
  return HRESULT_FROM_WIN32(status);
}
...
public:
  ByteArrayWrapper() { reset(); }

  ByteArrayWrapper(_Out_writes_(cbMax) UINT8 *pb, UINT32 cbMax) { reset(pb, cbMax); }
  ...
  void reset(_Inout_updates_bytes_(cbMax) UINT8 *pb, UINT32 cbMax)
  {
    m_pb    = pb;
    m_cbMax = cbMax;
    m_cbCur = 0;
  ...
  void clear() { m_cbCur = 0; }

  UINT32 size() const { return m_cbCur; }

  BYTE const &front() const { return *m_pb; }

  BYTE const *data() const { return m_pb; }

  UINT32 capacity() const { return m_cbMax; }
  ...
  {
    static UINT8 dummy;
    return index < m_cbMax ? m_pb[index] : dummy;
  }
};
...
template <class ByteBufferTy>
class ProviderMetadataBuilder : public detail::MetadataBuilderBase<ByteBufferTy>
{
  /*
  ProviderMetadata =
...
public:
  ...
  template <class CharTy>
  void Begin(_In_z_ CharTy const *szUtfProviderName)
  {
    this->BaseBegin();
    this->AddString(szUtfProviderName);
  }

  void AddTrait(ProviderTraitType type, _In_bytecount_(cbData) void const *pData, unsigned cbData)
  {
...
template <class ByteBufferTy>
class EventMetadataBuilder : public detail::MetadataBuilderBase<ByteBufferTy>
{
  /*
  EventMetadata =
  Size    : UINT16; // Size counts the whole data structure, including the Size field.
  ...
  Tags[]  : UINT8 (only present if OutMeta & OutMetaChain, read bytes until you encounter a byte
  with high bit unset); Ccount  : UINT16 (only present if InMeta & InMetaCountMask == InMetaCcount);
  ...
  */
  ...
  EventMetadataBuilder(ByteBufferTy &buffer, UINT32 bookmark)
      : detail::MetadataBuilderBase<ByteBufferTy>(buffer), m_bookmark(bookmark)
  {
    return;
  }
  ...
  void AddFieldInfo(UINT8 arity, Type type, UINT32 tags)
  {
    ...
    _TLD_ASSERT((type & _TLD_MAKE_TYPE(0, OutTypeMask)) == (Type)(type & 0xffffff00),
                "OutType out of range");
...
public:
  explicit EventMetadataBuilder(ByteBufferTy &buffer)
      : detail::MetadataBuilderBase<ByteBufferTy>(buffer), m_bookmark(0)
  {
    return;
  }

  EventMetadataBuilder(EventMetadataBuilder &other)
      : detail::MetadataBuilderBase<ByteBufferTy>(other.GetBuffer()), m_bookmark(other.m_bookmark)
  {
    return;
  }
  ...
  template <class CharTy>
  void Begin(_In_z_ CharTy const *szUtfEventName, UINT32 eventTags = 0)
  {
  ...
  template <class CharTy>
  EventMetadataBuilder AddStruct(_In_z_ CharTy const *szUtfStructName, UINT32 fieldTags = 0)
  {
  ...
  template <class CharTy>
  UINT32 AddStructRaw(_In_z_ CharTy const *szUtfStructName, UINT32 fieldTags = 0)
  {
    this->AddString(szUtfStructName);
    auto bookmark = AddStructInfo(InMetaScalar, fieldTags);
    return bookmark;
  ...
  template <class CharTy>
  EventMetadataBuilder AddStructArray(_In_z_ CharTy const *szUtfStructName, UINT32 fieldTags = 0)
  {
    this->AddString(szUtfStructName);
  ...
  template <class CharTy>
  EventMetadataBuilder AddStructFixedArray(_In_z_ CharTy const *szUtfStructName,
                                           UINT16 itemCount,
                                           UINT32 fieldTags = 0)
  {
    this->AddString(szUtfStructName);
  ...
  template <class CharTy>
  void AddField(_In_z_ CharTy const *szUtfFieldName, Type type, UINT32 fieldTags = 0)
  {
  ...
  template <class CharTy>
  void AddFieldArray(_In_z_ CharTy const *szUtfFieldName, Type type, UINT32 fieldTags = 0)
  {
    this->AddString(szUtfFieldName);
  ...
  template <class CharTy>
  void AddFieldFixedArray(_In_z_ CharTy const *szUtfFieldName,
                          UINT16 itemCount,
                          Type type,
                          UINT32 fieldTags = 0)
  {
  ...
  template <class CharTy>
  void AddFieldCustom(_In_z_ CharTy const *szUtfFieldName,
...
public:
  explicit EventDataBuilder(ByteBufferTy &buffer) : m_buffer(buffer) { return; }

  EventDataBuilder(EventDataBuilder &other) : m_buffer(other.m_buffer) { return; }

  EventDataBuilder(EventDataBuilder &&other) : m_buffer(other.m_buffer) { return; }

  ByteBufferTy &Buffer() { return m_buffer; }

  ByteBufferTy const &Buffer() const { return m_buffer; }

  void Clear() { m_buffer.clear(); }

  UINT32 Size() const { return static_cast<UINT32>(m_buffer.size()); }

  UINT8 &operator[](UINT32 index) { return m_buffer[index]; }

  void AddArrayCount(UINT16 cItems) { AddBytes(&cItems, sizeof(cItems)); }
  ...
  */
  ...
  Compatible with: TypeMbcsString, TypeMbcsXml, TypeMbcsJson, TypeUtf8String.
  */
  void AddString(_In_z_ char const *psz) { AddBytes(psz, static_cast<unsigned>(strlen(psz) + 1)); }
  ...
  */
  void AddByte(UINT8 val) { m_buffer.push_back(val); }
...
class Provider
{
  ...
  */
  ...
  static VOID NTAPI EnableCallback(_In_ LPCGUID pSourceId,
                                   ...
                                   _Inout_opt_ PVOID pCallbackContext)
  {
    if (pCallbackContext)
    {
      ...
      {
        pProvider->m_pfCallback(pSourceId, callbackType, level, keywordsAny, keywordsAll,
                                pFilterData, pProvider->m_pCallbackContext);
      }
    }
  }

  _Ret_opt_bytecount_(cbMetadata) static UINT8 *MetadataAlloc(UINT32 cbMetadata)
  {
    return static_cast<UINT8 *>(HeapAlloc(GetProcessHeap(), 0, cbMetadata));
  }

  static void MetadataFree(_Post_invalid_ UINT8 *pbMetadata)
  {
    if (pbMetadata)
    {
  ...
  void InitFail(HRESULT hr)
  {
    _TLD_ASSERT(m_pbMetadata != NullMetadata(),
                "InitFail called with m_pbMetadata == NullMetadata()");
  ...
  template <class CharTy>
  void Init(_In_z_ CharTy const *szName, _In_opt_ GUID const *pGroupId)
  {
  ...
  {
    return keywords == 0 ||
           ((keywords & m_keywordsAny) != 0 && (keywords & m_keywordsAll) == m_keywordsAll);
  }
...
public:
  ...
  Provider(_In_z_ wchar_t const *szUtf16Name,
           GUID const &providerId,
           _In_opt_ GUID const *pGroupId = 0,
           PENABLECALLBACK pfCallback    = 0,
           LPVOID pCallbackContext       = 0)
      : m_pfCallback(pfCallback), m_pCallbackContext(pCallbackContext), m_id(providerId)
  {
    Init(szUtf16Name, pGroupId);
  }

  Provider(_In_z_ char const *szUtf8Name,
           GUID const &providerId,
           _In_opt_ GUID const *pGroupId = 0,
           PENABLECALLBACK pfCallback    = 0,
           LPVOID pCallbackContext       = 0)
  ...
  explicit Provider(_In_z_ wchar_t const *szUtf16Name,
                    _In_opt_ GUID const *pGroupId = 0,
                    PENABLECALLBACK pfCallback    = 0,
                    LPVOID pCallbackContext       = 0)
  ...
  explicit Provider(_In_z_ char const *szUtf8Name,
                    _In_opt_ GUID const *pGroupId = 0,
                    PENABLECALLBACK pfCallback    = 0,
                    LPVOID pCallbackContext       = 0)
      : m_pfCallback(pfCallback),
  ...
  HRESULT InitializationResult() const { return m_hrInitialization; }

  _Ret_z_ char const *Name() const { return reinterpret_cast<char const *>(m_pbMetadata + 2); }

  GUID const &Id() const { return m_id; }

  _Ret_notnull_ UINT8 const *GetMetadata() const { return m_pbMetadata; }

  bool IsEnabled() const { return m_levelPlus1 != 0; }

  bool IsEnabled(UCHAR level) const { return level < m_levelPlus1; }
  ...
  HRESULT SetInformation(_EVENT_INFO_CLASS informationClass,
                         _In_reads_bytes_opt_(cbInformation) void *pbInformation,
                         ULONG cbInformation) const
  {
    return ::tld::SetInformation(m_hProvider, informationClass, pbInformation, cbInformation);
  }

  HRESULT WriteGather(EVENT_DESCRIPTOR const &eventDescriptor,
                      _In_count_x_((UINT16 *)pEventMetadata) UINT8 const *pEventMetadata,
                      _In_range_(2, 128) ULONG cDataDescriptors,
                      _Inout_count_(cDataDescriptors) EVENT_DATA_DESCRIPTOR *pDataDescriptors,
                      _In_opt_ LPCGUID pActivityId        = 0,
                      _In_opt_ LPCGUID pRelatedActivityId = 0) const
  {
    return ::tld::WriteEvent(m_hProvider, eventDescriptor, m_pbMetadata, pEventMetadata,
                             cDataDescriptors, pDataDescriptors, pActivityId, pRelatedActivityId);
  }

  HRESULT Write(EVENT_DESCRIPTOR const &eventDescriptor,
                _In_count_x_((UINT16 *)pEventMetadata) UINT8 const *pEventMetadata,
                ...
                _In_opt_ LPCGUID pRelatedActivityId = 0) const
  {
    ...
    return ::tld::WriteEvent(m_hProvider, eventDescriptor, m_pbMetadata, pEventMetadata,
                             cbData ? 3 : 2, dataDesc, pActivityId, pRelatedActivityId);
  }
};
...
protected:
  EventBuilder(ByteBufferTy &metaBuffer, ByteBufferTy &dataBuffer, EventState &eventState)
      : m_metaBuilder(metaBuffer), m_dataBuilder(dataBuffer), m_eventState(eventState)
  {
    return;
  }
...
public:
  ...
  */
  template <class CharTy>
  EventBuilder AddStruct(_In_z_ CharTy const *szUtfStructName, UINT32 fieldTags = 0)
  {
  ...
  */
  template <class CharTy>
  EventBuilder AddStructArray(_In_z_ CharTy const *szUtfStructName, UINT32 fieldTags = 0)
  {
    _TLD_ASSERT(m_eventState == EventStateOpen, "Metadata already prepared.");
  ...
  */
  template <class CharTy>
  EventBuilder AddStructFixedArray(_In_z_ CharTy const *szUtfStructName,
                                   UINT16 itemCount,
                                   UINT32 fieldTags = 0)
  {
    _TLD_ASSERT(m_eventState == EventStateOpen, "Metadata already prepared.");
    return EventBuilder(*this,
                        m_metaBuilder.AddStructFixedArray(szUtfStructName, itemCount, fieldTags));
  }
  ...
  */
  void AddRawMetadata(_In_reads_bytes_(cbMetadata) void const *pMetadata, UINT32 cbMetadata)
  {
  ...
  */
  void AddArrayCount(UINT16 cItems) { m_dataBuilder.AddArrayCount(cItems); }
  ...
  */
  ...
  /*
  Appends an InTypeAnsiString field to the payload.
  Compatible with: TypeUtf8String, TypeMbcsXml, TypeMbcsJson.
  */
  void AddString(_In_z_ char const *psz) { m_dataBuilder.AddString(psz); }

  /*
  Appends a InTypeUnicodeString field to the payload.
  Compatible with: TypeUtf16String, TypeUtf16Xml, TypeUtf16Json
  */
  void AddString(_In_z_ wchar_t const *psz) { m_dataBuilder.AddString(psz); }

  /*
  Appends a raw byte to the payload.
  */
  void AddByte(UINT8 val) { m_dataBuilder.AddByte(val); }
...
template <class ByteBufferTy>
class Event : public EventBuilder<ByteBufferTy>
{
...
public:
  Event(_In_z_ char const *szUtf8Name,
        UCHAR level                    = 5,
  ...
  Event(_In_z_ wchar_t const *szUtf16Name,
        UCHAR level                    = 5,
        ULONGLONG keywords             = 0,
        UINT32 tags                    = 0,
        UCHAR opcode                   = 0,
  ...
  */
  template <class CharTy>
  void Reset(_In_z_ CharTy const *szUtfName,
             UCHAR level              = 5,
             ULONGLONG keywords       = 0,
             UINT32 tags              = 0,
             UCHAR opcode             = 0,
  ...
  */
  void ResetData() { m_dataBuffer.clear(); }
  ...
  */
  HRESULT Write(Provider const &provider)
  {
    ...
    {
      hr = provider.Write(
          m_descriptor, &m_metaBuffer.front(), m_dataBuffer.empty() ? NULL : &m_dataBuffer.front(),
          static_cast<UINT32>(m_dataBuffer.size()), m_pActivityId, m_pRelatedActivityId);
    }
    return hr;
  }
  ...
  Sends the event to ETW using EventWriteTransfer.
  */
  HRESULT Write(REGHANDLE hProvider,
                _In_count_x_((UINT16 *)pProviderMetadata) UINT8 const *pProviderMetadata)
  {
    HRESULT hr = E_INVALIDARG;
    ...
    if (PrepareEvent())
    {
      ...
      {
        EventDataDescCreate(&dataDesc[cDataDesc++], &m_dataBuffer.front(),
                            static_cast<ULONG>(m_dataBuffer.size()));
      }

      hr = ::tld::WriteEvent(hProvider, m_descriptor, pProviderMetadata, &m_metaBuffer.front(),
                             cDataDesc, dataDesc, m_pActivityId, m_pRelatedActivityId);
    }
...
#pragma region Basic properties
  ...
  */
  UINT32 Identity() const { return m_descriptor.Identity(); }

  /*
  Note: identity is a 24-bit value, containing descriptor.Id + descriptor.Version.
  */
  void Identity(UINT32 identity) { m_descriptor.Identity(identity); }

  UCHAR Channel() const { return m_descriptor.Channel; }
  ...
  */
  void Channel(UCHAR channel) { m_descriptor.Channel = channel; }

  UCHAR Level() const { return m_descriptor.Level; }

  void Level(UCHAR value) { m_descriptor.Level = value; }

  ULONGLONG Keywords() const { return m_descriptor.Keyword; }

  void Keywords(ULONGLONG value) { m_descriptor.Keyword = value; }

  UCHAR Opcode() const { return m_descriptor.Opcode; }

  void Opcode(UCHAR value) { m_descriptor.Opcode = value; }

  USHORT Task() const { return m_descriptor.Task; }

  void Task(USHORT value) { m_descriptor.Task = value; }

  GUID *ActivityId() const { return m_pActivityId; }

  void ActivityId(GUID *pValue) { m_pActivityId = pValue; }

  GUID *RelatedActivityId() const { return m_pRelatedActivityId; }

  void RelatedActivityId(GUID *pValue) { m_pRelatedActivityId = pValue; }
...
private:
  ...
  {
    m_state =
        EventMetadataBuilder<ByteBufferTy>(m_metaBuffer).End() ? EventStateClosed : EventStateError;
  }
};
