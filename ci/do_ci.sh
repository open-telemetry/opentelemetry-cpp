...
EOF
  ...
  # there are some exceptions and error handling code from the Prometheus and Jaeger Client
  # that make this test always fail. ignore Prometheus and Jaeger exporter in the noexcept here.
  bazel $BAZEL_STARTUP_OPTIONS build --copt=-fno-exceptions $BAZEL_OPTIONS --build_tag_filters=-jaeger -- //... -//exporters/prometheus/...
  bazel $BAZEL_STARTUP_OPTIONS test --copt=-fno-exceptions $BAZEL_TEST_OPTIONS --build_tag_filters=-jaeger -- //... -//exporters/prometheus/...
  exit 0
